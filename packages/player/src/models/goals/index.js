...
const ShowActions = ({ history }) => (
  <TopToolbar>
    <Button color="primary" label="Back" onClick={goBack(history)}>
      <ChevronLeft />
    </Button>
  </TopToolbar>
);
...
const GoalShow = ({ dataProvider, classes, history, ...rest }) => {
  const [teams, setTeams] = useState([]);

  useEffect(() => {
    const getTeams = async () => {
      try {
        const res = await dataProvider(GET_LIST, constants.resources.teams, {}).then(
          (res) => res.data,
        );
        setTeams(res);
      } catch (e) {
