  ...
  render() {
    ...
    return (
      <div className={label && classes.approveButtonWithLabel}>
        ...
        <IconButton onClick={this._handleApprove} disabled={record.status === 'approved'}>
          <ThumbUp className={record.status === 'approved' ? classes.approved : ''} />
        </IconButton>
        <IconButton onClick={this._handleReject} disabled={record.status === 'rejected'}>
