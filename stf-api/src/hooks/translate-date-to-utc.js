    ...
    if (context.params.query && context.params.query[dateKey]) {
      if (typeof context.params.query[dateKey] === 'object') {
        if (context.params.query[dateKey].$gt) {
          context.params.query[dateKey].$gt = localDateAsUTC(
            context.params.query[dateKey].$gt,
            offset,
          );
        }
        if (context.params.query[dateKey].$gte) {
          context.params.query[dateKey].$gte = localDateAsUTC(
            context.params.query[dateKey].$gte,
            offset,
          );
        }
        if (context.params.query[dateKey].$lt) {
          context.params.query[dateKey].$lt = localDateAsUTC(
            context.params.query[dateKey].$lt,
            offset,
          );
        }
        if (context.params.query[dateKey].$lte) {
          context.params.query[dateKey].$lte = localDateAsUTC(
            context.params.query[dateKey].$lte,
            offset,
          );
        }
      } else {
